...
// Deactivate club
$stmt = $pdo->prepare("UPDATE Club SET club_status = 'inactive' WHERE club_id = ?");
$ok   = $stmt->execute([$clubId]);

if ($ok) {
    $_SESSION['toast_message'] = 'Club deactivated successfully.';
} else {
    $_SESSION['toast_message'] = 'Failed to deactivate club.';
}
